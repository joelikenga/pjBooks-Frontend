...
export const Navbar = () => {
  ...
  const inputValueFunction = (e: ChangeEvent<HTMLInputElement>) => {
    let text = e.target.value;
    SetInputValue(text);
  };

  // input check for space and focus on input

  const handleFocus = () => {
    if (inputRef.current) {
      const value = inputRef.current.value;
      // Check if value consists only of spaces or tabs
      if ((value && /^[ \t]*$/.test(value)) || inputFocus === true) {
        SetInputFocus(false);
      }
    }
  };
  ...
  const handleBlur = () => {
    ...
    if (inputRef.current) {
      ...
      // Check if value is empty or consists only of spaces or tabs
      if ( value.length >= 0 ) {
        SetInputFocus(true);
      }
    }
          ...
          <div className="flex justify-center items-center gap-4 md:gap-[3rem]">
            ...
            <div className="flex justify-between items-center gap-4 md:gap-6">
              ...
              <div className={""} onClick={searchClick}>
                <button
                  className={
                    "block md:hidden text-sm justify-center items-center border border-default_secondary text-white md:text-default_secondary bg-default_secondary md:bg-white hover:bg-default_secondary hover:text-white duration-300 rounded-lg px-2.5 py-1.5 cursor-pointer md:text-xl"
                  }
                >
                  <BiSearch />
                </button>
          ...
          {openSearch && (
            ...
            >
              ...
              <motion.div
                ...
                className="mt-6 flex justify-between w-[80%] items-center bg-white rounded-lg "
              >
                <div className="m-1 w-full">
                  <div className="flex justify-between w-full items-center px-2">
                    <div className=" p-1 rounded flex gap-2 justify-start items-center w-full">
              ...
              {/* ------------ conditional popular subjects display section and search result display section---------------*/}

              {inputFocus ? (
                <motion.div
                  variants={staggerVariants}
                  onClick={handleClickInsideModal}
                  className=" mt-4 w-full h-[78%] overflow-y-scroll flex flex-col over"
                >
                  <SubjectsSection />
                  {/* //subject */}
                </motion.div>
                ...
                <motion.div
                  variants={staggerVariants}
                  onClick={handleClickInsideModal}
                  className=" mt-1 w-full h-[100%]  flex flex-col justify-start"
                >
                  <div className="flex justify-center ">
                    <p className="text-center text-ellipsis truncate text-white w-[15em]">
